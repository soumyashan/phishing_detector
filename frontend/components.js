...
function renderAIAnalysisCard(aiAnalysis) {
  if (!aiAnalysis) {
    return `
      <div class="card">
        <div class="card-header">
          <div class="card-title">
            ...
            AI/ML Detection Analysis
          </div>
        </div>
        <div class="card-body">
          <div class="empty-state">
            <i class="fa-solid fa-circle-info"></i>
            <div class="empty-state-title">AI/ML analysis unavailable</div>
            <div class="empty-state-text">
              Run an email analysis to generate AI/ML detection results.
            </div>
          </div>
        </div>
      </div>
  ...
  const prediction = String(aiAnalysis.prediction || "UNKNOWN").toUpperCase();

  const finalRiskNumber = Number(aiAnalysis.riskScore);
  const phishingProbabilityNumber = Number(aiAnalysis.phishingProbability);
  const aiRiskNumber = Number(aiAnalysis.aiRiskScore);
  const securityRiskNumber = Number(aiAnalysis.securityRiskScore);

  const finalRisk = Number.isFinite(finalRiskNumber)
    ? finalRiskNumber.toFixed(1)
    : "—";

  const phishingProbability = Number.isFinite(phishingProbabilityNumber)
    ? (phishingProbabilityNumber * 100).toFixed(1)
    : "—";

  const aiRisk = Number.isFinite(aiRiskNumber)
    ? aiRiskNumber.toFixed(1)
    : "—";

  const securityRisk = Number.isFinite(securityRiskNumber)
    ? securityRiskNumber.toFixed(1)
    : "—";
  ...
  const reasons = Array.isArray(aiAnalysis.reasons)
    ? aiAnalysis.reasons
    : [];
    ...
    : `
        ...
        ">
          <i
            ...
            "
          ></i>
          <span>No significant phishing indicators were returned by the AI/ML analysis.</span>
        </li>
      `;
  ...
  return `
    <div class="card">
      <div class="card-header">
        ...
        ${renderBadge(predictionClass, prediction)}
      </div>
      ...
      <div class="card-body">
        ...
        >
          ...
          <div class="meta-item">
            <div class="meta-label">Final Risk Score</div>
            <div class="meta-value">
              ${finalRisk}<span style="font-size:12px;opacity:.65;">/100</span>
            </div>
          </div>

          <div class="meta-item">
            <div class="meta-label">Phishing Probability</div>
            <div class="meta-value">
              ${phishingProbability}%
            </div>
          </div>
          ...
          <div class="meta-item">
            <div class="meta-label">AI/SVM Risk</div>
            <div class="meta-value">
              ${aiRisk}<span style="font-size:12px;opacity:.65;">/100</span>
            </div>
          </div>

          <div class="meta-item">
            <div class="meta-label">Security Risk</div>
            <div class="meta-value">
              ${securityRisk}<span style="font-size:12px;opacity:.65;">/100</span>
            </div>
          </div>
